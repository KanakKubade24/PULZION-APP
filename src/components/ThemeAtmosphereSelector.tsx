...
import { AlienThemeId } from '../types/theme';
...
export const ThemeAtmosphereSelector: React.FC = () => {
  ...
  const visualModes: { id: BgVisualMode; name: string; icon: any; desc: string }[] = [
    {
      id: 'curved-waves',
  ...
  return (
    <>
      {/* Full Modal Drawer for Themes & Background Curvature */}
      {isAtmosphereModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-black/80 backdrop-blur-md animate-in fade-in duration-300">
          ...
          >
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs sm:text-sm font-chakra text-slate-300">
                    Switch between 6 organic curved themes & background fluid simulations
                  </p>
                </div>
              </div>
            ...
            {/* SECTION 1: 6 Alien Themes Selection */}
            <div className="mt-6">
                  ...
                  return (
                    ...
                    >
                          ...
                          {theme.palette.extraColors ? (
                            <>
                              <span 
                                ...
                                style={{ backgroundColor: theme.palette.extraColors.starlightGold }} 
                                title="#f8d092 Starlight Gold"
                              />
                              <span 
                                className="w-3.5 h-3.5 rounded-full shadow-sm" 
                                style={{ backgroundColor: theme.palette.extraColors.seaGreen }} 
                                title="#38a48c Alien Sea Green"
                              />
                              <span 
                                className="w-3.5 h-3.5 rounded-full shadow-sm" 
                                style={{ backgroundColor: theme.palette.extraColors.cyanTeal }} 
                                title="#159097 Nebula Cyan"
                              />
                              <span 
                                className="w-3.5 h-3.5 rounded-full shadow-sm" 
                                style={{ backgroundColor: theme.palette.extraColors.deepTeal }} 
                                title="#1a5678 Deep Teal"
                              />
                              <span 
                                className="w-3.5 h-3.5 rounded-full shadow-sm" 
                                style={{ backgroundColor: theme.palette.extraColors.cosmicIndigo }} 
                                title="#25245d Cosmic Indigo"
                              />
                              <span 
                                className="w-3.5 h-3.5 rounded-full shadow-sm" 
                                style={{ backgroundColor: theme.palette.extraColors.deepPurple }} 
                                title="#571c56 Deep Purple"
                              />
                            </>
                          ) : (
                            <>
                      ...
                      <div className="flex items-start justify-between gap-2">
                        ...
                        {isSelected && (
                          <div 
                            className="w-6 h-6 rounded-full flex items-center justify-center text-black font-bold shadow-md"
                            style={{ backgroundColor: theme.palette.primaryHex }}
                          >
                    ...
                    >
                      ...
                      <div className="flex-1">
                        <div className="flex items-center justify-between">
                          <p className="font-chakra font-bold text-sm text-white">
                            {mode.name}
                          </p>
                          {isActive && (
                            <span 
